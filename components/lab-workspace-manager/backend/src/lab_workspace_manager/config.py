...
class WorkspaceManagerSettings(BaseSettings):
    # Settings passed by docknet
    SELF_ACCESS_URL: str = Field("", env="DOCKNET_SERVICE_URL")
    SELF_DEPLOYMENT_NAME: str = Field("", env="DOCKNET_DEPLOYMENT_NAME")
    ...
    WORKSPACE_IMAGE_DEFAULT: WorkspaceImage = WorkspaceImage(
        image="khulnasoft/ml-workspace-minimal", display_name="Default Minimal Workspace"
    )
